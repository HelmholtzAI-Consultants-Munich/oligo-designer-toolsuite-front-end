...
const GenomeAlignmentD3 = {
    create: (el: Element, oligos: any[], selectedOligo: number, setSelectedOligo: (index: number) => void) => {
        const width = 800;
        const height = 200;
        const margin = 20;
        ...
        const ext = d3.extent(oligoPositions.flatMap(d => [d.start, d.end])) as [number, number]
        ...
        plot.append("g")
            ...
            .on("click", (_, data) => {
                console.log(data);
                const index = oligoPositions.findIndex(pos => pos.id === data.id);
        ...
        function zoomed(event: d3.D3ZoomEvent<Element, unknown>) {
            ...
            plot.selectAll<SVGRectElement, typeof oligoPositions[0]>(".oligos rect")
                .attr("x", d => zx(d.start))
    ...
    update: (el: Element, oligos: Oligo[], selectedOligo: number) => {
        ...
        svg.selectAll<SVGRectElement, typeof oligoPositions[0]>(".oligos rect")
            .data(oligoPositions)
            .join("rect")
