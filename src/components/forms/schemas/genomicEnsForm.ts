...
const form_Data_Ens: EnsFastaFormDataGeneric<true> = {
    ...
    source: {
        value: "ensembl",
        comment: "required: indicate that ensembl annotation should be used",
    },
    source_params: {
        species: {
            value: "homo_sapiens",
            comment:
                "required: species name in ensemble download format, e.g. 'homo_sapiens' for human; see http://ftp.ensembl.org/pub/release-108/gtf/ for available species names",
        },
        annotation_release: {
            value: "current",
            comment:
                "required: release number of annotation, e.g. 'release-108' or 'current' to use most recent annotation release. Check out release numbers for ensemble at ftp.ensembl.org/pub/",
        },
    },
    genomic_regions: {
