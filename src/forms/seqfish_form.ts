const formDatas = {
    ...
    dir_output: { value: "output_seqfishplus_probe_designer", comment: "name of the directory where the output files will be written" },
    ...
    files_fasta_target_probe_database: { value: ["data/genomic_regions/cds_annotation_source-NCBI_species-Homo_sapiens_annotation_release-110_genome_assemly-GRCh38.fna, data/genomic_regions/utr_annotation_source-NCBI_species-Homo_sapiens_annotation_release-110_genome_assemly-GRCh38.fna"], comment: "fasta file with sequences form which the probes should be generated. Hint: use the genomic_region_generator pipeline to create fasta files of genomic regions of interest" },
    files_fasta_reference_database_target_probe: { value: ["data/genomic_regions/exon_annotation_source-NCBI_species-Homo_sapiens_annotation_release-110_genome_assemly-GRCh38.fna, data/genomic_regions/exon_exon_junction_annotation_source-NCBI_species-Homo_sapiens_annotation_release-110_genome_assemly-GRCh38.fna"], comment: "fasta file with sequences used as reference for the specificity filters. Hint: use the genomic_region_generator pipeline to create fasta files of genomic regions of interest" },
    target_probe_length_min: { value: "28", comment: "min length of probe" },
    target_probe_length_max: { value: "28", comment: "max length of probe" },
    target_probe_isoform_consensus: { value: "100", comment: "min isoform consesnsus for probes, i.e. how many transcripts of the total number of transcripts of a gene are covered by the probe, given in %" },
    ...
    target_probe_UTR_weight: { value: "10", comment: "weight of the Tm of the probe in the efficiency score" },
    set_size_min: { value: "24", comment: "minimum size of probe sets (in case there exist no set of the optimal size) -> genes with less oligos will be filtered out and stored in regions_with_insufficient_oligos_for_db_probes" },
    set_size_opt: { value: "24", comment: "optimal size of probe sets" },
    distance_between_target_probes: { value: "2", comment: "how much overlap should be allowed between oligos, e.g. if oligos can overlpap x bases choose -x, if oligos can be next to one another choose 0, if oligos should be x bases apart choose x" },
    ...
    readout_probe_GC_content_max: { value: "60", comment: "maximum GC content of oligos" },
    ...
    primer_number_GC_GCclamp: { value: "1", comment: "the minimum number of G or C nucleotides required within the specified number of bases" },
    primer_number_three_prime_base_GCclamp: { value: "2", comment: "the number of bases to consider from the 3' end of the sequence" },
    ...
    target_probe_specificity_blastn_hit_parameters: {
        min_alignment_length: { value: "15", comment: "can be turned into coverage" }
    },
    ...
    readout_probe_specificity_blastn_hit_parameters: {
        min_alignment_length: { value: "10", comment: "can be turned into coverage" }
    },
