    ...
    def run(self, region_form: dict[str, Any]) -> list[str]:
        ...
        with output_lock.write_lock():
            ...
            if source_val == "ensembl":
                ...
                cache_info = EnsemblGenomicDataBase(cache_dir=self.cache_dir).fetch_genomic_entity(
                    genomic_entity
                )
                files_source = "Ensembl"
            else:
                # Default to NCBI second-line cache
                cache_info = NCBIGenomicDataBase(cache_dir=self.cache_dir).fetch_genomic_entity(
                    genomic_entity
                )
                files_source = "NCBI"
