...
def oligoseq():
    """
    Handles the OligoSeq probe designer requests by preparing user inputs, managing temporary files,
    ...
    and a run ID. It orchestrates the workflow for running the OligoSeq probe designer pipeline as follows:
    ...
    - Launches the external `OligoSeq_probe_designer` process as a subprocess, passing the YAML config.
    ...
    For more information on the input parameters and configuration options, refer to the OligoSeq documentation.

    """
    # 1. Set user directory and config path based on authentication status
    user_dir = ''
    ...
    # 3. Convert run_id to ObjectId and handle errors
    try:
    ...
    # 4. Handle "file_regions" input: convert comma list to temp .txt if needed
    ...
    # 5. Prepare output path and update DB run record
    ...
    # 6. Build config for the designer
    config = {
    ...
    # 7. Write YAML config file for probe designer
    with open(config_path, "w") as f:
        yaml.dump(config, f, sort_keys=False)

    # 8. Run the designer subprocess and collect output
    ...
    # 9. Cleanup: delete temp region files and fasta files created for this run
    if os.path.exists(form_data['file_regions']['value']):
        print('deleted')
        os.remove(form_data['file_regions']['value'])  # Delete region file

    # Delete uploaded fasta files (target probe database)
