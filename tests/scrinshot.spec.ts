...
test.describe("Scrinshot Page", () => {
    ...
    test("should render required file inputs", async ({ page }) => {
        ...
        await expect(
            page.locator('input[name="files_fasta_target_probe_database"]')
        ).toBeVisible();
        await expect(
            page.locator(
    ...
    test("should display and fill numeric input fields by name", async ({
        page,
    }) => {
        const inputNames = [
            "file_regions",
    ...
    test("clicking Generate FASTA+ adds new form", async ({ page }) => {
        await page
            .getByRole("button", { name: "Generate FASTA+" })
            .first()
            .click();
    ...
    test("fills all visible inputs in all tabs including Developer Settings", async ({
        page,
    }) => {
