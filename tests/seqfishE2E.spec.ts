...
test("E2E: submit seqfish form with gene and FASTA files", async ({ page }) => {
    ...
    const fastaTargetInput = page.locator(
        'input[name="files_fasta_target_probe_database"]'
    );
    ...
    const fastaRefInput = page.locator(
        'input[name="files_fasta_reference_database_target_probe"]'
    );
    ...
    await page
        .getByRole("button", { name: "Readout Probe Parameters" })
        .click();
    await expect(
        page.getByRole("button", { name: "Readout Probe Parameters" })
    ).toHaveClass(/active/);

    // 5. Upload reference probe database FASTA
    const fastaRefProbeInput = page.locator(
        'input[name="files_fasta_reference_database_readout_probe"]'
    );
    ...
    await expect(
        page.getByRole("button", { name: "Primer Parameters" })
    ).toHaveClass(/active/);

    // 5. Upload reference probe database FASTA
    const fastaRefPrimerInput = page.locator(
        'input[name="files_fasta_reference_database_primer"]'
    );
    ...
    expect(
        gotSuccess,
        'Expected a final "Pipeline is successfully finished" alert'
    ).toBeTruthy();
});
